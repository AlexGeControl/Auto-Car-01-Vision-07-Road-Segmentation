...
def test_optimize(optimize):
    ...
    logits, train_op, cross_entropy_loss, mean_iou_op, mean_iou = optimize(layers_output, correct_label, learning_rate, num_classes)
...
def test_train_nn(train_nn):
    ...
    mean_iou_op = tf.constant(0) 
    mean_iou = tf.constant(0)
...
def test_for_kitti_dataset(data_dir):
    kitti_dataset_path = os.path.join(data_dir, 'kitti')
